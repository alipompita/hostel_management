<?php 
	...
	// 	$regNo=$_POST["regNo"];
	// 	$year=$_POST["year"];
	// 	// $conn->query("INSERT INTO Allocated(reg_no,yearAllocated) VALUES('$regNo',$year)") or die($conn->error);
	// }
...
<body>
	<main class="container-fluid">
		...
		<section class="container-fluid">
			<div class="row">
				<div class="col-md-12">
					<center><h2>Room Allocation</h2></center>
				</div>
			</div>
			<div class="row">
				...
				<div class="side-nav" style="border: solid black red;">
					...
					<li><a class="active" href="applicationList.php">Application List</a></li>
					  <li><a class="active" href="allocatedList.php"><strong>Allocated List</strong></a></li>
				...
				<div class="col-md-9">
					<!-- <h2>Student Allocation List</h2> -->
					<?php
						$all=$conn->query("SELECT RB.*,S.*,H.*,R.* FROM RoomBookings RB INNER JOIN Student S ON RB.studentId=S.reg_no INNER JOIN HostelTbl H ON RB.hostelId=H.hostel_id INNER JOIN Room R ON RB.roomId=R.room_id ") or die($conn->error);
						if ($all->num_rows>0) {
							?>
						<table class="table table-all" id="myTable">
							...
							<form class="" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="post">
									...
									<td><?php echo "Year ".$row["room_name"]; ?></td>
									<td><a href="">Change</a></td>
									<td><a href="">Disallocate</a></td>
							...
							echo "No data for room allocation is available";
						}
					 ?>
				</div>
			</div>
